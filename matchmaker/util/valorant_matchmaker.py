...
from matchmaker.util.valorant_team import ValorantTeam
...
class ValorantMatchMaker:
    ...
    def _build_starting_dataset(self):
        for player in self._players:
            ...
            if len(player_rankings) < 2:
                ...
                self._player_rankings_map[player.username] = player.average_valorant_ranking_adjusted

    def _get_complete_pool_for_role(self, role: VALORANT_ROLE, from_player_pool=None) -> List[User]:
    ...
    def _get_primary_pool(self, role: VALORANT_ROLE, from_player_pool=None) -> List[User]:
        player_pool = self._players if from_player_pool is None else from_player_pool

        return [player for player in player_pool if player.valorant_role_preferences.primary_role.value == role.value]

    def _get_secondary_pool(self, role: VALORANT_ROLE, from_player_pool=None) -> List[User]:
        player_pool = self._players if from_player_pool is None else from_player_pool

        return [player for player in player_pool if player.valorant_role_preferences.secondary_role.value == role.value]

    def _get_off_pool(self, role: VALORANT_ROLE, from_player_pool=None) -> List[User]:
        player_pool = self._players if from_player_pool is None else from_player_pool

        return [player for player in player_pool if player.valorant_role_preferences.off_role.value == role.value]
    ...
    def _get_matchup_skew(self, matchup: Matchup, role: Union[VALORANT_ROLE, FLOAT_ROLE]) -> Tuple[float, Matchup]:
        first_user_rating = matchup[0].average_valorant_ranking_adjusted_for_role(role)
        second_user_rating = matchup[1].average_valorant_ranking_adjusted_for_role(role)
    ...
    def _raise_for_unmatchable_dataset(self):
        complete_controller_pool = self._get_complete_pool_for_role(VALORANT_ROLE.CONTROLLER)
        complete_duelist_pool = self._get_complete_pool_for_role(VALORANT_ROLE.DUELIST)
        complete_initiator_pool = self._get_complete_pool_for_role(VALORANT_ROLE.INITIATOR)
        complete_sentinel_pool = self._get_complete_pool_for_role(VALORANT_ROLE.SENTINEL)

        controller_count = len(complete_controller_pool)
        duelist_count = len(complete_duelist_pool)
        initiator_count = len(complete_initiator_pool)
        sentinel_count = len(complete_sentinel_pool)
        ...
        if any(
                [
                    not controller_count >= 2,
                    not duelist_count >= 2,
                    not initiator_count >= 2,
                    not sentinel_count >= 2,
                ]
        ):
            count_detail = f"top: {controller_count}, jng: {duelist_count} mid: {initiator_count}, adc: {sentinel_count}"
            raise UnMatchableStateException(f"Not enough players within each role to create a team. {count_detail}")

        return None

    def _find_best_matchup_from_pool(self, pool: List[User], role: Union[VALORANT_ROLE, FLOAT_ROLE]) -> Tuple[float, Matchup]:
    ...
    def _matchmake(
            self, team_a: ValorantTeam, team_b: ValorantTeam, player_pool: list[User], roles_to_fill: list[VALORANT_ROLE]
    ) -> Tuple[list[User], list[VALORANT_ROLE]]:
        if len(player_pool) == 2:
            # Last loop, we can skip some things. We know that there is only one more role to fill
            # For valorant the last player will be a filler role, they can do whatever they want. We just assume
            # they will play their primary role
            role_to_fill = FLOAT_ROLE
    ...
    def matchmake(self) -> Tuple[ValorantTeam, ValorantTeam]:
        ...
        while attempt < max_attempts:
            ...
            roles_to_fill = [role for role in VALORANT_ROLE]
